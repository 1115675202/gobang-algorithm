...
public class AiAlgorithms {
	...
	private static List<PatternEnum>[] calculateAllPattern(Position position) {
		...
		for (DirectionEnum direction : DirectionEnum.values()) {
			PatternEnum ownPattern =
					calculateBy(direction, position, ownColor, opponentColor);
			patternGroups[0].add(ownPattern);
			PatternEnum opponentPattern =
					calculateBy(direction, position, opponentColor, ownColor);
	...
	private static PatternEnum calculateBy(
		...
		int[] subjectArray = findSubjectArray(
				direction, originOfPosition, ownColor, opponentColor);
		return PatternAlgorithms.check(subjectArray);
	}
	...
	private static int[] findSubjectArray(
		...
		subjectList.add(Integer.valueOf(O));
		...
		for (int offsetNum = 1; (toInitialPoint || awayInitialPoint) && offsetNum <= maxOffsetNum; offsetNum++) {
			if (toInitialPoint) {
				...
				subjectList.addFirst(Integer.valueOf(chessInfoType));
			}
			...
			if (awayInitialPoint) {
				...
				subjectList.addLast(Integer.valueOf(chessInfoType));
			}
		}
